...
class ChartingState extends MusicBeatState
{
	...
	**/
		...
		if (!blockInput)
		{
			...
			if (FlxG.mouse.wheel != 0)
			{
				...
				if (!mouseQuant)
				...
				else
					{
						...
						if (FlxG.mouse.wheel > 0)
						{
							var fuck:Float = CoolUtil.quantize2(beat, snap) - increase;
			...
			if(!vortex){
				if (FlxG.keys.justPressed.UP || FlxG.keys.justPressed.DOWN  )
				{
					...
					{
						var fuck:Float = CoolUtil.quantize2(beat, snap) - increase; //(Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(fuck);
					}else{
						var fuck:Float = CoolUtil.quantize2(beat, snap) + increase; //(Math.floor((beat+snap) / snap) * snap);
						FlxG.sound.music.time = Conductor.beatToSeconds(fuck);
	...
	function setupSusNote(note:Note, beats:Float):FlxSprite {
		...
		var spr:FlxSprite = new FlxSprite(note.x + (GRID_SIZE * 0.5) - 4, note.y + GRID_SIZE / 2).makeGraphic(8, height, noteColors[note.noteData%noteColors.length]);
		return spr;
	}
