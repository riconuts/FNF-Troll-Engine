...
class PlayState extends MusicBeatState
{
	...
	////
	public var showDebugTraces:Bool = Main.showDebugTraces;
	...
	*/
	...
	function checkCharacterDance(character:Character, ?beat:Float, ignoreBeat:Bool = false){
		...
		if (shouldBop || ignoreBeat){
			...
			if ((character.animation.finished || !character.animation.curAnim.name.startsWith("sing")) && !character.stunned) 
				character.dance();
		}
	...
	override public function update(elapsed:Float)
	{
		...
		*/
		...
		if (generatedMusic && !isDead) {
			...
			for (field in playfields) {
				var holdingKeys = field.keysPressed.contains(true);
				for (char in field.characters) {
					if (char.canResetDance(holdingKeys)) {
						char.resetDance();
						// trace("reset");
					}
				}
	...
	private function displayJudgment(image:String) {
		...
		var spr:RatingSprite = worldCombos ? ratingGroup.displayJudgment(image, -40, -60) : ratingGroup.displayJudgment(image, ClientPrefs.comboOffset[0], -ClientPrefs.comboOffset[1]);

		if (ClientPrefs.simpleJudge) {
			if (lastJudge != null) lastJudge.kill();
	...
	private function displayCombo(?combo:Int){
		...
		lastCombos = (worldCombos) ? ratingGroup.displayCombo(combo, 0, 80) : ratingGroup.displayCombo(combo, ClientPrefs.comboOffset[2], -ClientPrefs.comboOffset[3]);
